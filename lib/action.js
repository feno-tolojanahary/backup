...
const S3Manager = require("./s3/s3");
const s3Manager = new S3Manager();
const remoteS3 = require('./s3/remoteS3')
...
const { getNewPassword, unlockPrompt, printTable, printS3Configs, printHostConfigs } = require("./helper/ui-console");
...
const oneDownloadFromAllStorage = async (name, isFolder, dest) => {
    ...
    if (isFolder) {
        ...
        if (!existsSync(downloadPath)) {
            await s3Manager.downloadPrefix(name, dest)
        }
        return dest;
    } else {
        downloadPath = await remoteHandler.downloadFileFromServer(name, null, dest);
        if (!existsSync(downloadPath)) {
            await s3Manager.downloadFile(name, dest);
        }
        return dest;
    }
}
...
exports.removeArchives = async () => {
    ...
    await remoteS3.removeS3Archives(s3Archives);
...
exports.removeBackup = async (name, opts) => {
    ...
    if (opts.wasabi) {
        delOkS3 = await remoteS3.removeS3Archive(name);
    } else if (opts.remote) {
...
exports.backupList = async (opts) => {
    if (opts.wasabi) {
        if (!(await remoteS3.testConnection())) {
            console.log("List empty. Not connected to the s3 server");
            return;
        }
        let fileList = await remoteS3.listS3Backup();
        if (fileList) {
...
exports.checkHealth = async (name, opts) => {
    ...
    const wasabiConfigExists = remoteS3.wasabiConfigExists();
    const hasS3Connected = await remoteS3.testConnection();
...
exports.resetStorage = async (opts) => {
    if (opts.wasabi || opts.all) {
        const fileList = await remoteS3.listS3Backup();
        if (fileList) {
            const archives = fileList.map((file) => ({ storagePath: file.Key }));
            await remoteS3.removeS3Archives(archives);
...
const plainManualBackup = async (cmd, opts) => {
    ...
    if (opts.wasabi && remoteS3.wasabiConfigExists()) {   
        await s3Manager.createBucketIfNotExists({ bucket: config.wasabi.bucketName });
        await remoteS3.copyBackupToS3(backupFile);
    }
...
const encryptedManualBackup = async (cmd, opts) => {
    ...
    if (!opts.remote && remoteS3.wasabiConfigExists()) {
        await remoteS3.encryptedBackupToS3(dumpData);
    } 
...
exports.listConfigs = async () => {
    ...
    hosts = hosts.map(async (config) => {
        try {
            const remote = new RemoteHost(config);
