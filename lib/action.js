...
exports.removeBackup = async (name, opts) => {
    ...
    if (opts.wasabi) {
        delOkS3 = await s3Handler.removeS3Archive(name, { configName: opts.s3Config });
    } else if (opts.remote) {
...
exports.backupList = async (opts) => {
    if (opts.wasabi) {
        if (!(await s3Handler.testConnection(opts.s3Config))) {
            console.log("List empty. Not connected to the s3 server");
            return;
        }
        let fileList = await s3Handler.listS3Backup({ configName: opts.s3Config });
        if (fileList) {
...
const restoreEncryptedBackup = async (name, opts) => {
    let downloadPath = "";
    if (opts.wasabi) {
        downloadPath = await s3Wasabi.downloadPrefix(name, TEMP_DIR, opts.s3Config);
    } else if (opts.remote) {
...
const restorePlainBackup = async (backupInfo, opts) => {
    let downloadPath = "";
    if (opts.wasabi) {
        downloadPath = await s3Wasabi.downloadFile({ key: backupInfo.name, configName: opts.s3Config });
    } else if (opts.remote) {
...
exports.checkHealth = async (name, opts) => {
    ...
    const wasabiConfigExists = s3Handler.wasabiConfigExists(opts.s3Config);
    const hasS3Connected = await s3Handler.testConnection(opts.s3Config);
...
exports.resetStorage = async (opts) => {
    if (opts.wasabi || opts.all) {
        const fileList = await s3Handler.listS3Backup({ configName: opts.s3Config });
        if (fileList) {
            const archives = fileList.map((file) => ({ storagePath: file.Key }));
            await s3Handler.removeS3Archives(archives, { configName: opts.s3Config });
...
const plainManualBackup = async (cmd, opts) => {
    ...
    if (opts.wasabi && s3Handler.wasabiConfigExists(opts.s3Config)) {   
        await s3Wasabi.createBucketIfNotExists({ configName: opts.s3Config });
        await s3Handler.copyBackupToS3(backupFile, { configName: opts.s3Config });
    }
...
const encryptedManualBackup = async (cmd, opts) => {
    ...
    if (!opts.remote && s3Handler.wasabiConfigExists(opts.s3Config)) {
        await s3Handler.encryptedBackupToS3(dumpData, { configName: opts.s3Config });
    } 
