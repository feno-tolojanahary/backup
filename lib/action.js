...
const restoreEncryptedBackup = async (name, opts) => {
    ...
    if (opts.wasabi) {
        ...
        downloadPath = await remoteHandler.downloadDirFromServer(name, path.join(TEMP_DIR, name), opts.host);
    }
    const res = await sendIpcRequest({ msg: "decyprt", payload: downloadPath });
...
exports.lockVault = async () => {
    const res = await sendIpcRequest({ action: "lock" });
