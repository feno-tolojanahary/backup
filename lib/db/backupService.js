...
class BackService {
    ...
    async insert(backup) {
        try {
            ...
            const result = insert.run(
                backupUid, name, userId, encrypted, status, modifiedAt, storage, isSynced, lastSynced, storageConf, job_run_id, destinationId, jobId
            )
            ...
            for (const upFile of uploadedData) {
                const backupFile = {
                    ...upFile,
                    prefix,
                    status: status === 'failed' ? 'failed' : "online",
        ...
        try {
            let query = `
                SELECT
                    ...
                    b.storage AS storage,
                    b.storage_conf AS storageConf,
                    b.archived_date AS archivedDate,
                    json_group_array(
                        json_object(    
                            'fileId', bf.id,
                            'storagePath', bf.storage_path,
                            'backupId', bf.backup_id,
                            'size', bf.size,
                            'status', bf.status,
                            'type', bf.type,
                            'prefix', bf.prefix,
                            'destinationFolder', bf.destination_folder
                        )
                    ) AS files,
                    json_object (
                        'id', j.id,
            ...
            const backupData = rows.map((backup) => ({ 
                ...backup,    
                files: backup.files ? JSON.parse(backup.files) : [],
                isEncrypted: backup.isEncrypted === 1,
                isSynced: backup.isSynced === 1,
                lastSynced: backup.lastSynced ? new Date(backup.lastSynced * 1000) : null,
                modifiedAt: backup.modifiedAt ? new Date(backup.modifiedAt * 1000) : null,
                createdAt: new Date(backup.createdAt * 1000),
                archivedDate: backup.archivedDate ? new Date(backup.archivedDate * 1000) : null,
            }))
            return {
                success: true,
                data: backupData
            ...
            console.log("error get backup list: ", error);
            return {
                success: false,
                message: error.message
            }
    ...
    async existsByStoragePath (backupstoragePath) {
        try {
            const query = `
                SELECT backup_id FROM backup_files WHERE storage_path = ? LIMIT 1
            `
            const stmt = db.prepare(query);
        ...
        try {
            const stmt = db.prepare(`
                SELECT
                    ...
                    b.storage_conf AS storageConf,
                    b.job_id AS jobId,
                    b.job_run_id AS jobRunId,
                    b.archived_date AS archivedDate,
                    b.destination_id AS destinationId,
                    bf.id AS fileId,
            ...
            console.log("Error find by name backup: ", error);
            throw error;
        }
    }
    ...
    async deleteBackupFiles (filters = {}) {
    ...
    async deleteExpiredBackupByJob(job, remoteInstances) {
        ...
        for (const backup of backups) {
            try {
                ...
                if (backup.storage === "local-storage") {
                    for (const backupFile of backupFiles) {
                        const localStorage = new LocalStorage(backup.storage_conf);
                        await localStorage.deleteEntry(backupFile);
                    }
                }
    ...
    async getSizeSumBackupFile (storageConf) {
        try {
            const res = db.prepare(`
                SELECT SUM(bf.size) AS totalSize FROM backup_files bf
                LEFT JOIN backups b ON b.id = bf.backup_id
                WHERE b.storage_conf = ?
            `).get(storageConf);
            return {
                success: true,
    ...
    async getLastBackupFiles (storageConf, { limit = 10 }) {
        try {
            const backupData = db.prepare(`
                    SELECT SUM(bf.size) AS totalSize, b.id AS backup_id, GROUP_CONCAT(bf.storage_path, ';') AS storagePaths FROM backup_files bf
                    RIGHT JOIN backups b ON b.id = bf.backup_id
                    WHERE b.storage_conf = ?
                    GROUP BY b.id
                    ORDER BY b.created_at DESC
                    LIMIT ?
                `).all(storageConf, limit);
            return backupData;
        } catch (error) {
            console.log("Error get last backup file: ", error.message)
            return [];
        }
    }
}
