    ...
    async insert(job) {
        try {
            ...
            const stmt = db.prepare(`INSERT INTO jobs (
                    name, is_enable, status, storage, schedule_type, schedule_value, created_by, created_at, updated_at, source_config_name, dest_config_name
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);

            const res = stmt.run(name, isEnable, status, storage, scheduleType, scheduleValue, createdBy, createdAt, createdAt, sourceConfigName, destConfigName);
