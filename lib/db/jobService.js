    ...
    async insert(job) {
        try {
            ...
            const stmt = db.prepare(`INSERT INTO jobs (
                    name, is_enable, status, schedule_type, schedule_value, created_by, created_at, updated_at, target
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);

            const res = stmt.run(name, isEnable, status, scheduleType, scheduleValue, createdBy, createdAt, createdAt, target);
