const { default: CronExpressionParser } = require("cron-parser");
const db = require("../db");

class JobService {
    constructor() {}

    async insert(job) {
        try {
            const {
                name,
                status,
                scheduleType,
                scheduleValue,
                createdBy,
                target,
                targetId,
                isEncrypted
            } = job;

            const createdAt = Math.floor(Date.now() / 1000);
            const isEnable = 1;
            const encrypted = isEncrypted ? 1 : 0;

            const stmt = db.prepare(`INSERT INTO jobs (
                    name, is_enable, status, schedule_type, schedule_value, created_by, created_at, updated_at, target, target_id, is_encrypted
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);

            const res = stmt.run(name, isEnable, status, scheduleType, scheduleValue, createdBy, createdAt, createdAt, target, targetId, encrypted);
            if (!res.lastInsertRowid)
                throw new Error("Unable to insert job data");
            console.log("res insertion job: ", res)
            return {
                success: true,
                lastID: res.lastInsertRowid,
                message: "Job inserted with success"
            }
    } catch (error) {
            console.log("Error inserting job: ", error.message)
            return {
                success: false,
                message: error.message
            }
        }
    }

    async update(filters = [], updateData) {
        try {
            const setUpdate = [], paramVals = [];
            
            if (!filters.name && !filters.id)
                return;
          
            const allowUpdateMapFields = {
                "name": "name",
                "isEncrypted": "is_encrypted",
                "retentionDays": "retention_days",
                "isEnable": "is_enable",
                "scheduleType": "schedule_type",
                "scheduleValue": "schedule_value",
                "sourceConfigName": "source_config_name",
                "destConfigName": "dest_config_name"
            }   

            
            for (let [key, value] of Object.entries(updateData)) {
                if (allowUpdateMapFields[key]) {
                    setUpdate.push(` ${allowUpdateMapFields[key]} = ? `);
                    if (["isEncrypted", "isEnable"].includes(key))
                        value = value ? 1 : 0;
                    paramVals.push(value);
                }
            }

            if (setUpdate.length === 0)
                throw new Error("No update fields.");

            let query = `
                UPDATE jobs
                SET ${setUpdate.join(", ")}
                WHERE 1=1
            `;

            
            if (filters.name) {
                query += " AND name = ?";
                paramVals.push(filters.name);
            }

            if (filters.id) {
                query += " AND id = ?";
                paramVals.push(filters.id);
            }
            console.log("query: ", query)  
            const updStmt = db.prepare(query)
            
            const res = updStmt.run(...paramVals);
            if (res.changes === 0) 
                throw new Error("Jobs not found.")
            return {
                success: true,
                message: "Job updated with success."
            }
        } catch (error) {
            return {
                success: false,
                message: error.message
            }
        } 
    }

    async deleteJob (filters) {
        try {
            if (!filters.id && !filters.name) {
                throw new Error("the params jobId or jobName is required");
            }
            let query = `DELETE FROM jobs WHERE 1 = 1`
            if (filters.id) {
                query += ` AND id = ?`;
            } 
            if (filters.jobName) {
                query += ` AND name = ?`;
            }
            const paramVals = [filters.id, filters.name].filter(Boolean);
            const stmt = db.prepare(query);
            const res = stmt.run(...paramVals);
            return {
                changes: res.changes,
                success: true,
                message: "Job deleted with success."
            }
        } catch (error) {
            console.log("error delete job: ", error.message)
            return {
                success: false,
                message: error.messge
            }    
        }
    }

    async listJob (filters) {
        try {
            const paramVals = [];
            let query = `SELECT * FROM jobs WHERE 1=1`;
            if (filters.status) {
                query += " AND status = ?"
                paramVals.push(filters.status)
            }
            if (filters.userId) {
                query += " AND created_by = ?";
                paramVals.push(filters.userId)
            }
            const stmt = db.prepare(query);
            const jobs = stmt.all(...paramVals);
            return {
                success: true,
                count: jobs.length,
                data: jobs.map((j) => ({
                    id: j.id,
                    name: j.name,
                    isEnable: j.is_enable,
                    status: j.status,
                    storage: j.storage ? JSON.parse(j.storage) : [],
                    scheduleType: j.schedule_type,
                    scheduleValue: j.schedule_value,
                    createdBy: j.created_by,
                    updatedAt: j.updated_at
                }))
            }
        } catch (error) {
            console.log("Error get list job: ", error.message)
            return {
                success: false,
                message: error.message
            }
        }
    }

    async findJob (filters) {
        try {
            let paramVals = [];
            let query = `SELECT * FROM jobs WHERE 1=1`;
            if (filters.id) {
                query += ` AND id = ?`;
                paramVals.push(filters.id);
            }
            const job = db.prepare(query).get(...paramVals);
            return job;
        } catch (error) {
            return null;
        }
    }

    async getListJobs(filters) {
        try {
            let query = `
                SELECT 
                    j.id AS id, 
                    j.name AS name,
                    j.status AS status,
                    j.schedule_type AS scheduleType,
                    j.schedule_value AS scheduleValue,
                    j.created_by AS createdBy,
                    j.is_encrypted AS isEncrypted,
                    j.retention_days AS rententionDays,
                    (CASE
                        WHEN s.id NOT NULL THEN
                        json_object(
                            'id', s.id,
                            'name', s.name,
                            'config', s.config
                        ) 
                    END) AS source,
                    json_group_array(
                        CASE
                            WHEN d.id IS NOT NULL THEN
                            json_object(
                                'id', d.id,
                                'name', d.name
                            )
                        END
                    ) AS destinations,
                    (SELECT 
                        json_object(
                            'finishedAt', jr.finished_at,
                            'status', jr.status
                        )
                        FROM job_runs jr 
                            WHERE jr.job_id = j.id ORDER BY jr.finished_at DESC LIMIT 1) AS lastJobRun 
                FROM jobs j 
                    LEFT JOIN job_schedule_state j_s ON j_s.job_id = j.id
                    LEFT JOIN targets t ON t.id = j.target_id
                    LEFT JOIN sources s ON s.id = t.source_id
                    LEFT JOIN target_destinations td ON td.target_id = t.id
                    LEFT JOIN destinations d ON d.id = td.destination_id
                WHERE 1=1
                `;
            const params = [];
                
            if (filters.userId) {
                query += " AND j.created_by = ?";
                params.push(filters.userId)
            }
            query += " GROUP BY j.id"
            const jobs = db.prepare(query).all(...params);
            return jobs.map((job) => ({
                ...job,
                source: job.source ? JSON.parse(job.source) : null,
                destinations: job.destinations ? JSON.parse(job.destinations).filter(Boolean) : [],
                isEncrypted: Boolean(job.isEncrypted),
                lastJobRun: job.lastJobRun ? { ...JSON.parse(job.lastJobRun), finishedAt: new Date(JSON.parse(job.lastJobRun).finishedAt * 1000) } : null
            }));
        } catch(error) {
            console.log("Error get list: ", error.message)
            return []
        }
    }

    async getJobDetail(id) {
        try {
            let query = `
                SELECT 
                    j.id AS id, 
                    j.name AS name,
                    j.status AS status,
                    j.schedule_type AS scheduleType,
                    j.schedule_value AS scheduleValue,
                    j.created_by AS createdBy,
                    j.is_encrypted AS isEncrypted,
                    j.retention_days AS rententionDays,
                    (CASE
                        WHEN s.id NOT NULL THEN
                        json_object(
                            'id', s.id,
                            'name', s.name,
                            'type', s.type,
                            'config', s.config
                        )
                    END) AS source,
                    json_group_array(
                        CASE
                            WHEN d.id IS NOT NULL THEN
                            json_object(
                                'id', d.id,
                                'name', d.name,
                                'type', d.type,
                                'config', d.config
                            )
                        END
                    ) AS destinations
                FROM jobs j 
                    LEFT JOIN job_schedule_state j_s ON j_s.job_id = j.id
                    LEFT JOIN targets t ON t.id = j.target_id
                    LEFT JOIN sources s ON s.id = t.source_id
                    LEFT JOIN target_destinations td ON td.target_id = t.id
                    LEFT JOIN destinations d ON d.id = td.destination_id
                WHERE 1=1 j.id = ?
                GROUP BY j.id
            `;
            const job = db.prepare(query).get(id);
            return {
                ok: true,
                job: {
                    ...job,
                    source: job.source ? JSON.parse(job.source) : null,
                    destinations: job.destinations ? JSON.parse(job.destinations).filter(Boolean) : [],
                    isEncrypted: Boolean(job.isEncrypted),
                    target: JSON.parse(job.target)
                }
            }
        } catch (error) {
            return {
                ok: false,
                errorMsg: error.message
            }
        }
    }

    async getNextRunJob(timeStampSecond) {
        try {
            const query = `
                SELECT * FROM job_schedule_state jbsc
                LEFT JOIN jobs j ON j.id = jbsc.job_id
                WHERE jbsc.next_run_at <= ?
                AND j.is_enable = 1
            `
            const stmt = db.prepare(query);
            const jobs = stmt.all(timeStampSecond);
            return jobs
        } catch (error) {
            return [];
        }
    }

    async pushToNextRun(jobs) {
        const finishedDate = Math.floor(Date.now()/1000);
        for (const job of jobs) {
            try {
                const stmt = db.prepare("UPDATE job_schedule_state SET next_run_at = ?, last_run_at = ? WHERE id = ?");
                let nextRun = job.next_run_at;
                switch(job.schedule_type) {
                    case "interval":
                        nextRun = finishedDate + Number(job.schedule_value);
                        break;
                    case "cron":
                        const interval = CronExpressionParser.parse(job.schedule_value, { currentDate: new Date(finishedDate*1000) });
                        nextRun = Math.floor(interval.next().getTime() / 1000);
                        break;
                    default:
                        break;
                }
                stmt.run(nextRun, finishedDate, job.id);
            } catch (error) {
                console.log("Update next run job: ", error.message);
            }
        }
    }

    async getNextToLaunch() {
        try {
            const currentDate = Date.now() / 1000;
            const stmt = db.prepare(`
                SELECT MIN(jb_s.next_run_at) AS next_run_at FROM job_schedule_state jb_s
                LEFT JOIN jobs j ON j.id = jb_s.job_id
                WHERE j.is_enable = 1 AND jb_s.next_run_at > ? 
            `)
            const nextJob = stmt.get(currentDate);
            return nextJob;
        } catch (error) {
            console.log("Error happen when get next to launch: ", error.message)
        }
    }

    async getRentetionJobs () {
        try {
            const jobs = db.prepare(`SELECT * FROM jobs WHERE retention_days IS NOT NULL AND retention_days > 0`).all();
            return jobs;
        } catch (error) {
            console.log("Error getting expirated jobs: ", error.message);
            return []
        }
    }

    async createJobRun (jobRun) {
        try {
            const {
                id,
                jobId,
                status,
                startAt = Date.now() / 1000,
                finishedAt,
                errorCode,
                errorMessage,
                createdAt = Date.now() / 1000,
            } = jobRun;

            const stmt = db.prepare(`INSERT INTO job_runs (
                id, job_id, status, started_at, finished_at, error_code, error_message, created_at    
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
            const res = stmt.run(id, jobId, status, startAt, finishedAt, errorCode, errorMessage, createdAt);
            return {
                success: true,
                lastId: res.lastInsertRowid                
            }
        } catch (error) {
            console.log("Error inserting job run: ", error.message)
            return {
                success: false,
                message: error.message
            }
        }
    }

    async updateJobRun (jobRunId, update) {
        try {
            const setUpdateQr = Object.keys(update).map((key) => ({ attr: `${key} = ?`, value: update[key] }));
            const setUpdate = setUpdateQr.map(({ attr }) => attr);
            const values = setUpdateQr.map(({ value }) => value);
            const stmt = db.prepare(`UPDATE job_runs SET ${setUpdate.join(', ')} WHERE id = ?`);
            const res = stmt.run(...values, jobRunId);
            if (res.changes === 0)
                throw new Error("No job_runs data to update found.");
            return {
                success: true
            }
        } catch (error) {
            return {
                success: false,
                message: error.message
            }
        }
    }

    async getJobRuns (filters) {
        try {
            let whereVals = [];
            let query = `
                SELECT 
                    jr.id as id,
                    jr.job_id as jobId,
                    jr.status as status,
                    jr.started_at as startedAt,
                    jr.finished_at as finishedAt,
                    jr.error_code as errorCode,
                    jr.error_message as errorMessage,
                    jr.created_at as createdAt,
                    j.name as jobName,
                    (SELECT COUNT(*) FROM backups b WHERE b.job_id = j.id) as backupCount
                FROM job_runs jr 
                LEFT JOIN jobs j ON j.id = jr.job_id
                WHERE 1=1 `;
            if (filters.jobId) {
                query += " AND jr.job_id = ?";
                whereVals.push(filters.jobId);
            }

            if (filters.status) {
                query += " AND jr.status = ?";
                whereVals.push(filters.status);
            }
            if (filters.since) {
                query += " AND jr.created_at > ?";
                whereVals.push(filters.since);
            }
            query += " ORDER BY jr.finished_at DESC";
            const jobRuns = db.prepare(query).all(...whereVals);
            return jobRuns.map((run) => ({
                ...run,
                startAt: run.startAt ? new Date(run.startAt * 1000) : null,
                finishedAt: run.finishedAt ? new Date(run.finishedAt * 1000) : null,
                createdAt: run.createdAt ? new Date(run.createdAt * 1000) : null
            }))
        } catch (error) {
            console.log("Eror when getting list of job runs: ", error.message);
            return [];  
        }
    }

    async getLastJobRunId() {
        const lastJobRun = db.prepare("SELECT id FROM job_runs ORDER BY id DESC LIMIT 1").get();
        return lastJobRun.id;
    }

    async setStatusJob(jobId, status) {
        try {
            db.prepare("UPDATE jobs SET status = ? WHERE id = ?").run(status, jobId);
        } catch (error) {
            console.log("Error set status job: ", error.message)
        }
    }
}

module.exports = new JobService();