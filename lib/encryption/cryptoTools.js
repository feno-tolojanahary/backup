...
exports.encryptFile = async (filePath) => {
    ...
    const masterKey = await vaultSession.getMasterKey();
...
const deriveMasterKey = async (passwordKey) => {
...
const getHashedPassword = (password, salt) => argon2.hash(password, {
    type: argon2.argon2id,
    salt,
    memoryCost: 64 * 1024,
    timeCost: 3,
...
const getVaultData = (passwordKey, masterKey, salt) => {
...
exports.generateVaultFile = async (password) => {
    ...
    const passwordKey = await getHashedPassword(password, salt);
    const vaultData = getVaultData(passwordKey, masterKey, salt)

    await writeFile(VAULT_FILE_PATH, JSON.stringify(vaultData));
    return passwordKey;
}

exports.updateVaultPasswordKey = async (oldPassword, newPassword) => {
    const masterKey = await deriveMasterKey(oldPassword);
    const passwordKey = await getHashedPassword(newPassword);
    const vaultData = getVaultData(passwordKey, masterKey)
