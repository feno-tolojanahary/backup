...
async function uploadToStorage ({conf, dumpMetaData, job, isEncrypted}) {
    ...
    if (conf.type === "ssh") {
        try {
            ...
            backupEvent.emit("backup_success", { uploadedData: [backupInfo], ...otherMeta, job_run_id: job.job_run_id, storage: "remote" })
        } catch (error) {
            console.log("Error ssh upload: ", error)
            backupEvent.emit("backup_failed", { uploadedData: [dumpMetaData], ...otherMeta, job_run_id: job.job_run_id, storage: "remote" });
            throw new Error(error.message);
        }
    }
    if (conf.type === "s3") {
        try {
            ...
            backupEvent.emit("backup_success", { ...dumpMetaData, uploadedData, ...otherMeta, storage: "s3", job_run_id: job.job_run_id })
        } catch (error) {
            console.log("Error upload s3: ", error);
            backupEvent.emit("backup_failed", { ...dumpMetaData, ...otherMeta, job_run_id: job.job_run_id, storage: "s3" })
            throw new Error(error.message);
        }
    }
    if (conf.type === "local-storage") {
        try {
            ...
            backupEvent.emit("backup_success", { ...dumpMetaData, uploadedData, ...otherMeta, job_run_id: job.job_run_id, storage: "local-storage" })
        } catch (error) {
            console.log("Error copy local-storage: ", error);
            backupEvent.emit("backup_failed", { ...dumpMetaData, ...otherMeta, job_run_id: job.job_run_id, storage: "local-storage" })
            throw new Error(error.message);
        }
    }
}
