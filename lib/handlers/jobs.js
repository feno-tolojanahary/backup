...
async function uploadToStorage ({conf, dumpMetaData, job, isEncrypted}) {
    ...
    if (conf.type === "ssh") {
        try {
            ...
            backupEvent.emit("backup_success", { uploadedData: [backupInfo], ...otherMeta, storage: "remote" })
        } catch (error) {
            console.log("Error ssh upload: ", error)
            backupEvent.emit("backup_failed", { uploadedData: [dumpMetaData], ...otherMeta, storage: "remote" });
