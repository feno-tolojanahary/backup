...
const ConsoleError = require('./../../helper/consoleError');
...
const { archivePath, getDirSize } = require("../../helper/utils");
...
class MongodbManager {
    ...
    databaseExists = async (dbName) => {
        try {
            const client = new MongoClient(this.config.uri, { connectTimeoutMS: 5000 });
            await client.connect();
            const adminDb = client.db().admin();
            const { databases } = await adminDb.listDatabases();
            const exists = databases.some(db => db.name === this.config.database);
            if (!exists)
                throw new Error(`Database ${dbName} does not exists.`);
            return true;
        } catch (err) {
            throw err;
        }
    ...
    plainDumpMongoDb = async (outName = null) => {
        outName = outName || this.config.database;
        const { stdout, stderr } = await exec(`mongodump --db ${this.config.database} -o ${outName}`, 
    ...
    dumpMongoDb = async (outName) => {
        try {
            ...
            return { 
                storagePath: path.basename(encryptedDir),
                originalName: name,
                size: stat.size / 1024,
                encrypted: true,
            ...
            console.log("Error dump mongodb: ", error.code);
            if (error.code ===  'ENOENT') {
                throw new ConsoleError("The tool `mongodb` is not available on the system", 'ENOENT');
            }
            throw error;
        }
    }

    restoreMongoDb = async (zipPath, dbName, { originalName } = {}) => {
        ...
        const folderPath = path.join(__dirname, "../../../", dstFolderPath, originalName ?? dbName);
