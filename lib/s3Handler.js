...
exports.removeS3Archives = async (archives) => {
    if (archives.length > 0) {
        ...
        for (const archive of archives) { 
            try {
                await s3Wasabi.deleteFile(archive.name);
                console.log("should delete the key: ", key)
...
exports.encryptedBackupToS3 = async (metaBackup) => {
    try {
        ...
        const metaFiles = files.map(file => ({ filePath: path.join(metaBackup.encryptedDirPath, file.name), key: path.basename(backupInfo.encryptedDirPath) + "/" + file.name }));
        const uploadFiles = metaFiles.map(({ key, filePath }) => s3Wasabi.uploadFile({ filePath, key }));
        metaBackup.s3Keys = metaFiles.map(({ key }) => key);
