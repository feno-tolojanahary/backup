...
exports.removeS3Archives = async (archives) => {
    if (archives.length > 0) {
        ...
        for (const archive of archives) { 
            try {
                await s3Wasabi.deleteFile(archive.storagePath);
...
exports.encryptedBackupToS3 = async (metaBackup) => {
    try {
        ...
        const metaFiles = files.map(file => ({ filePath: path.join(metaBackup.encryptedDirPath, file.storagePath), key: path.basename(backupInfo.encryptedDirPath) + "/" + file.storagePath }));
