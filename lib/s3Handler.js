...
exports.removeS3Archive = async (name) => {
    try {
        let removePromise = [];
        const backups = await backupService.getAllBackups({ name, storage: "wasabi" });
        if (backups.length === 0) {
            const existWasabi = await s3Wasabi.checkFileExists(name);
            if (!existWasabi) {
