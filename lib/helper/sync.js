...
const { resolveLocalConfig, resolveRemoteConfig, resolveS3Config } = require("./mapConfig");
...
const getNativeList = async ({ synchronise = true, s3Config = "", remoteConfig = "", localStorageConfig } = {}) => {
    ...
    if (localHandler.hasValidConfig()) {
        ...
        localList = localList.map((meta) => ({ ...meta, type: "local-storage" }));
    }
    if (synchronise) {
...
const syncLogList = async ({ s3List, remoteList, localList }, destConfig) => {
    ...
    if (Array.isArray(s3List) && s3List.length > 0) {
        ...
        for (const meta of s3List) {
            const storagePath = meta.Key.replace(destConfig.backupPrefix || "", '');
    ...
    if (Array.isArray(remoteList) && remoteList.length > 0) {
        ...
        for (const meta of remoteList) {
            const storagePath = meta.key.replace(destConfig?.destinationFolder ?? "", '');
...
const getMetadataBackupList = async () => {
    const adminUser = await userService.adminUser();
...
exports.getMetadataBackupList = getMetadataBackupList;

exports.printBackupList = async () => {
    const backupList = await getMetadataBackupList();
