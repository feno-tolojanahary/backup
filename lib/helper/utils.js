...
const { readdir, stat, writeFile, unlink } = require("node:fs/promises");
...
exports.getDirSizeStream = async (root) => {
    const stacks = [root];
    let totalSize = 0;
    ...
    while(stacks.length > 0) {
        ...
        for (const dirent of dir) {
            if (dirent.isDirectory()) {
                stacks.push(path.join(current, dirent.name));
...
exports.verifyFolderWritable = async (folderPath) => {
    try {
        const testFileName = `.test-file-write-${crypto.randomUUID}`;
        await writeFile(path.join(folderPath), testFileName);
        await unlink(path.join(folderPath), testFileName);
        return {
            writable: true
        }
    } catch (error) {
...
exports.deleteEmptyFields = deleteEmptyFields; 
