...
const { removeLog } = require("./../logs/backupLog");
...
backupEvent.on("backup_success", async (metaBackup) => {
    ...
    const dataLog = {
        ...metaBackup,
        name: metaBackup.name ?? metaBackup.uploadedData?.[0]?.storagePath,
        status: "completed",
        userId: adminUser.id,
        encrypted: Boolean(metaBackup.encrypted) ? 1 : 0,
        modifiedAt: Date.now()
    }
    if (dataLog.storage === "s3") {
...
backupEvent.on("job_run_success", async (job, { startAt, finishedAt }) => {
    const jobRun = {
        id: job.job_run_id,
        jobId: job.id,
        status: "success",
    ...
    const resInsert = await jobService.createJobRun(jobRun)
}) 

backupEvent.on("job_run_failed", async (job, { startAt, finishedAt, errorMessage }) => {
    const jobRun = {
        id: job.job_run_id,
        jobId: job.id,
        status: "failed",
    ...
    await jobService.createJobRun(jobRun)
    if (config.notification.mail) {
        ...
        await sendEmailFailedBackup({provider, job: { name: job.name, date: startAt } })
    }
})
