...
function getS3Config (nameConf = null) {
    nameConf = `${nameConf ? nameConf : ""}`.trim();
...
function findTargetConf (targetName = null) {
    if (!Array.isArray(config.targets) || config.targets.length === 0) 
        throw new Error("No target configuration defined.");
    const targetConf = config.targets.find(({ name }) => name === targetName);
    if (!targetConf)
        throw new Error(`Target ${targetName} not found on the configuration.`);
    return targetConf;
}
...
function getConfigurationsByTargetName(targetName = null) {
    ...
    const sourceConf = foundTarget.type === "app" ? searchConfig(foundTarget.dbSource) : searchConfig(foundTarget.source);
    if (!sourceConf) 
        throw new Error(`No matching configuration source found for name ${foundTarget.source}`)
    return { target: foundTarget, destConfigs, sourceConf };
}

function allSourceConfs () {
    const sources = [
        ...config.infrastructure.sources?.mongodb || [],
        ...config.infrastructure.source?.s3 || []
    ];
    return sources;
}

function allDestinationConfs () {
    return [
        ...config.infrastructure.destinations?.s3 || [],
        ...config.infrastructure.destinations?.hosts || [],
        ...config.infrastructure.destinations?.localStorages || []
    ]
}
