...
exports.testConf = async (conf) => {
    ...
    switch(conf.type) {
        case "mongodb":
            await testMongodb(conf);
            break;
        case "object-replication":
            await testS3Provider(conf);
            break;
        case "ssh":
            await testHost(conf);
            break;
        case "local-storage":
            await testLocalStorage(conf);
            break;
        default:
...
exports.downloadBackup = async ({ backup, destParentPath = ""}) => {
    ...
    try {
        ...
        console.log(`Error download for ${filePath} on ${conf.name}`);
