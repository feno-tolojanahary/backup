...
const destinationService = require("../../api/services/infrastructure/destinationService");
...
exports.testConf = async (conf) => {
    ...
    const testS3Provider = async (conf) => {
        ...
        console.log("config s3 provider: ", conf)
        const status = await s3Provider.testConnection();
        console.log("test connection executed: ", s3Provider.testConnection())
    ...
    let resConf = {};
    if (!["mongodb", "s3", "ssh", "local-storage"].includes(conf.type))
        throw new Error("The config type %s is not supported", conf.type)
...
exports.deleteBackup = async (backup) => {
    ...
    if (backup.destinationId) {
        const destConf = await destinationService.findById(backup.destinationId);
        conf = { ...destConf, ...destConf.config };
        delete conf.config;
    } else {
        searchConfig(backup.storage_conf);
    }

    if (!["s3", "local-storage", "ssh"].includes(conf.type))
        throw new Error("The config type is not valid.");
    ...
    if (conf.type === "ssh") {
        const remoteHost = new RemoteHost(conf);
        await remoteHost.connect();
        console.log("files to delete: ", backup.files);
        const deleteEntries = backup.files.map((bFile) => remoteHost.removeEntry(bFile.storagePath, true));
        const resDel = await Promise.all(deleteEntries);
