...
const { entryExists, getDirSize, parseStorageSize } = require("../../helper/utils");
...
class LocalStorage {
    ...
    async deleteOverflowData (newBackupSize = 0) {
      if (!this.config.maxDiskUsage) 
        return;
      const resSum = await backupService.getSizeSumBackupFile(this.config.name);
      if (!resSum.success)
        return;
      const totalSizeStorage = resSum.totalSize;
      const maxDiskUsage = parseStorageSize(this.config.maxDiskUsage);
      if (maxDiskUsage > (totalSizeStorage + newBackupSize))
        return;
      // count the number of file to delete
      const upFrontSize = (totalSizeStorage + newBackupSize) - maxDiskUsage;
      const lastBackupData = await backupService.getLastBackupFiles();
      const totalSizeToRemove = 0;
      const toRemoveBackups = [];
      const toRemoveIndex = 0;
      while (upFrontSize > totalSizeToRemove) {
        const currentLastBackup = lastBackupData[toRemoveIndex];
        toRemoveBackups.push(currentLastBackup);
        toRemoveIndex++;
        totalSizeToRemove += currentLastBackup.totalSize;
      }
      
      for (const backup of toRemoveBackups) {
        const storagePaths = backup.storagePaths.split(";");
        const deleteEntries = storagePaths.map((sPath) => this.deleteEntry(sPath));
