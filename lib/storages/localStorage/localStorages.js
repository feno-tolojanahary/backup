...
class LocalStorage {
    ...
    async verifyFolderWritable(folderPath) {
      try {
        const testFileName = `.test-file-write-${crypto.randomUUID}`;
        await fs.writeFile(path.join(folderPath), testFileName);
        await fs.unlink(path.join(folderPath), testFileName);
        return {
          writable: true
        }
      } catch (error) {
    ...
    async destFolderWritable() {
      return this.verifyFolderWritable(this.destDirPath());
    }
}
