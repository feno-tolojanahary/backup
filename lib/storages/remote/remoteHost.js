...
class RemoteHost {
    ...
    checkFileExists (key) {
        return this.sftp.exists(path.posix.join(this.getWorkingDir(), key));
    }
    ...
    async ensurePrefixDirPath(prefixDirPath) {
        try {   
            const dir = path.join(this.getWorkingDir(), prefixDirPath);
    ...
    async uploadBuffer(buf, prefixDestFolder) {
        const dest = path.posix.join(this.getWorkingDir(), prefixDestFolder);
        const hasPermission = await this.#haveWritePermission(path.dirname(dest));
        if (!hasPermission) {
            console.log(`Unable to write on the following folder: ${this.getWorkingDir()}`);
            return;
        }
    ...
    async removeEntry(entryPath, isDir = false, destinationFolder = null) {
        try {
            const remoteWorkingDir = destinationFolder || this.getWorkingDir();
