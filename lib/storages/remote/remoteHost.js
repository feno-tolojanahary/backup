const fs = require("node:fs/promises");
const path = require("path");
// const { config } = require("./../../config");
const Client = require("ssh2-sftp-client");
const { decryptText } = require("../../../api/utils/cryptoKey");
const AppError = require("../../helper/error");

class RemoteHost {
    sftp = null;
    sshConfig = {};
    hostConfig = {};

    constructor(localConfig) {
        this.sftp = new Client();
        this.sshConfig = {
            host: localConfig.host,
            port: localConfig.port,
            username: localConfig.username,
            privateKeyEnc: localConfig.privateKeyEnc
        };
        if (localConfig.privateKey || localConfig.privateKeyEnc) {
            if (this.sshConfig?.privateKeyEnc) {
                localConfig.privateKey = decryptText(this.sshConfig.privateKeyEnc);
            }
            if (typeof localConfig.privateKey === "string" && localConfig.privateKey.includes("BEGIN")) {
                this.sshConfig.privateKey = localConfig.privateKey;
            } else {
                this.sshConfig.privateKey = path.join(__dirname, "../../../keys", localConfig.privateKey);
            }
            if (localConfig.passphrase) {
                this.sshConfig.passphrase = localConfig.passphrase;
            }
        } else if (localConfig.privateKeyPath) {
            this.sshConfig.privateKey = localConfig.privateKeyPath;
            if (localConfig.passphrase) {
                this.sshConfig.passphrase = localConfig.passphrase;
            }
        } else if (localConfig.password) {
            this.sshConfig.password = localConfig.password;
        }
        this.hostConfig = {...localConfig};
    }
    
    async connect () {
        try {
            if (this.sshConfig.privateKey && typeof this.sshConfig.privateKey === "string") {
                if (!this.sshConfig.privateKey.includes("BEGIN")) {
                    this.sshConfig.privateKey = await fs.readFile(this.sshConfig.privateKey);
                }
            }
            await this.sftp.connect(this.sshConfig);
        } catch (error) {
            throw new AppError(error);
        }
        // console.log("connected to remote host")
    }

    async isConnected () {
        try {
            await this.sftp.pwd();
            return true;
        } catch (error) {
            return false;
        }
    }

    getConfig() {
        return this.hostConfig;
    }

    getSftpClient() {
        return this.sftp;
    }

    checkFileExists (key) {
        return this.sftp.exists(path.posix.join(this.getRemoteWorkingDir(), key));
    }

    async testConnection() {
        try {
            await this.connect();
            await this.disconnect();
            return true;
        } catch (err) {
            return false;
        }
    }

    async disconnect () {
        await this.sftp.end();
        // console.log("deconnected to remote host")
    }
 
    async remoteDirExists (remoteDir) {
        try {
            await this.sftp.stat(remoteDir);
            return true;
        } catch (err) {
            if (err.code === "ENOENT") {
                return false;
            }
            throw err;
        }
    }

    async ensureRemoteDir(remoteDir) {
        try {   
            await this.sftp.stat(remoteDir);
        } catch (err) {
            if (err.code === "ENOENT") {
                await this.sftp.mkdir(remoteDir, true);
            } else {
                throw err; // unknown error
            }
        }
    }

    async ensurePrefixDirPath(prefixDirPath) {
        try {   
            const dir = path.join(this.getRemoteWorkingDir(), prefixDirPath);
            await this.sftp.stat(dir);
            return dir;
        } catch (err) {
            if (err.code === "ENOENT") {
                await this.sftp.mkdir(dir, true);
            } else {
                throw err; // unknown error
            }
        }
    }

    async #haveWritePermission(path) {
        try {
            const stats = await this.sftp.stat(path);
            const mode = stats.mode;
            const permissions = mode & 0o777;
            const ownerWritable = (permissions & 0o200) !== 0;
            const groupWritable = (permissions & 0o020) !== 0;
            const otherWritable = (permissions & 0o002) !== 0;

            return ownerWritable || groupWritable || otherWritable;
        } catch (err) {
            if (err.code === 2) {
                console.log(`Destination folder for ${path} doesn't exist`)
            } else if (err.code === "ENOENT") {
                console.log(`The path "${path}" does not exists`)
            } else {
                console.log(err.message);
            }
            return false;
        }
    }

    async getFolderSize(remotePath = null) {
        if (!remotePath) 
            remotePath = this.getRemoteWorkingDir();
        const list = await this.sftp.list(remotePath);
        let total = 0;

        for (const item of list) {
            const fullPath = `${remotePath}/${item.name}`;
            if (item.type === 'd') {
                total += await this.getFolderSize(fullPath); // recursive
            } else {
                total += item.size; // accumulate file sizes
            }
        }
        return total;
    }

    async getFileList(remotePath = null) {
        if (!remotePath) 
            remotePath = this.getRemoteWorkingDir();
        const fileList = await this.sftp.list(remotePath);
        return fileList;
    }

    getRemoteWorkingDir () {
        return this.hostConfig.destinationFolder || `/home/${this.hostConfig.username}/backup-data`;
    }

    async uploadFile(filePath, destFolder = null) {
        try {
            const remoteWorkingDir = destFolder || this.getRemoteWorkingDir();
            const hasPermission = await this.#haveWritePermission(path.dirname(remoteWorkingDir));
            if (!hasPermission) {
                console.log(`Unable to write on the following folder: ${remoteWorkingDir}`);
                return;
            }
            await this.ensureRemoteDir(remoteWorkingDir)
            const storagePath = path.basename(filePath);
            const dstPath = path.posix.join(remoteWorkingDir, storagePath);
            await this.sftp.fastPut(filePath, dstPath);
            console.log("Uploaded file to the server");
            return {
                dstPath,
                storagePath,
                type: "file",
                destinationFolder: remoteWorkingDir
            }
        } catch (error) {
            console.log(error);
            return;
        } 
    }

    async uploadBuffer(buf, prefixDestFolder) {
        const dest = path.posix.join(this.getRemoteWorkingDir(), prefixDestFolder);
        const hasPermission = await this.#haveWritePermission(path.dirname(dest));
        if (!hasPermission) {
            console.log(`Unable to write on the following folder: ${this.getRemoteWorkingDir()}`);
            return;
        }
        await this.ensureRemoteDir(dest)
        await this.sftp.put(buf, dest);
    }

    async uploadDir(dirPath, destFolder = null) {
    // note: use stream for the upload later
        try {
            const remoteFolder = destFolder || this.getRemoteWorkingDir();
            const hasPermission = await this.#haveWritePermission(path.dirname(remoteFolder));
            console.log("should continue here")
            if (!hasPermission) {
                console.log(`Unable to write on the following folder: ${remoteFolder}`);
                return;
            }
            await this.ensureRemoteDir(remoteFolder)
            const remotePath = path.posix.join(remoteFolder, path.basename(dirPath))
            const res = await this.sftp.uploadDir(dirPath, remotePath);
            return {
                dstPath: remotePath,
                storagePath: path.basename(dirPath),
                type: "folder",
                destinationFolder: remoteFolder
            };
        } catch (error) {   
            console.log("Error, upload dir remote: ", error.message);
            throw error;
        }
    }

    async removeEntry(entryPath, isDir = false, destinationFolder = null) {
        try {
            const remoteWorkingDir = destinationFolder || this.getRemoteWorkingDir();
            const remotePath = path.posix.join(remoteWorkingDir, path.basename(entryPath));
            const hasPermission = await this.#haveWritePermission(remotePath);
            if (!hasPermission) {
                throw new Error(`Error: permission denied, unable to deelte: ${remotePath}`);
            }
            let res;
            if (isDir) {
                console.log("the following remote path should be removed: ", remotePath)
                res = await this.sftp.rmdir(remotePath, true)
            } else {
                console.log("remove the following file: ", remotePath)
                res = await this.sftp.delete(remotePath);
            }
            return res; 
        } catch (error) {
            console.log("Error remove entry file: ", error);
            throw error;
        }
    }

    async downloadFile(fileName, localDestPath) {
        try {
            const workingDir = this.getRemoteWorkingDir();
            const filePath = path.posix.join(workingDir, fileName);
            await this.sftp.fastGet(filePath, localDestPath);
            return localDestPath;
        } catch (error) {
            console.log(error);
        } 
    }

    async downloadDir(dirName, localDestPath) {
        try {
            const workingDir = this.getRemoteWorkingDir();
            const filePath = path.posix.join(workingDir, dirName);
            await this.sftp.downloadDir(filePath, localDestPath);
            return localDestPath;
        } catch (error) {
            console.log(error);
        }
    }

    async uploadStream(stream, key) {
        try {
            const dirPath = await this.ensurePrefixDirPath(path.dirname(key));
            const filePath = path.posix.join(dirPath, path.fileName(key));
            const tempPath = path.posix.join(dirPath, `${filePath}.tmp`);
            await this.sftp.put(stream, tempPath, { flags: "w" });
            await this.sftp.rename(tempPath, filePath)
        } catch (error) {
            console.log("Upload stream ssh: ", error.message);
        }
    }
}

module.exports = RemoteHost;
