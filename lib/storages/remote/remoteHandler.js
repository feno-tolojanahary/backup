...
function getHostConfig(hostName) {
    let hostConf = {};
    if (hostName) {
        const foundConf = config.hosts.find((conf) => conf.name === hostName);
        if (!foundConf) {
...
const sendToRemoteServer = async ({hostConfig, metaBackup, isDir}) => {
    let filePath;
    let info;
    if (isDir) {
        filePath = metaBackup.encryptedDirPath;
    } else {
        filePath = path.join(hostConfig.workingDirectory, metaBackup.storagePath);
    }
    try {
        const remoteHost = new RemoteHost(hostConfig);
        await remoteHost.connect();
        if (isDir) {
            console.log("path to send: ", filePath)
            const destFolder = path.join(hostConfig.destinationFolder, path.basename(filePath));
            info = await remoteHost.uploadDir(filePath, destFolder);
            console.log("file is uploaded: ", info)
        } else {
            info = await remoteHost.uploadFile(filePath, hostConfig.destinationFolder);
        }
...
exports.sendToRemoteServers = async (metaBackup, { isDir = false } = {}) => {
    for (const hostConfig of config.hosts) {
        await sendToRemoteServer({hostConfig, metaBackup, isDir})        
    }
}

exports.downloadFileFromServer = async (fileName, hostConfName, dstPath = null) => {
    try {
        const hostConf = getHostConfig(hostConfName);
...
exports.downloadDirFromServer = async (dirName, hostConfName, dstPath) => {
    try {
        const hostConf = getHostConfig(hostConfName);
...
exports.removeArchive = async (nameId) => {
    ...
    if (resFind.success && resFind.data) {
        ...
        const res = await remoteHost.removeEntry(backup.storagePath, backup.isEncrypted);
...
exports.removeOverflowData = async (confName) => {
