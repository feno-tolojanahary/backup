...
// Lazy-load to avoid circular dependency with backupService -> S3Provider -> event -> backupService
const getBackupEvent = () => require("../../helper/event").backupEvent;
...
class S3Provider {
    ...
    async testConnection() {
        try {
            console.log("call head test connection")
            await this.s3.send(new HeadBucketCommand({ Bucket: this.bucketName }))
            console.log("result connect ok")
            return true;
        } catch (error) {
            console.log("Error test connection s3 provider: ", error);
            return false;
        }
    }
    ...
    removeS3Archive = async (nameId) => {
        try {
            ...
            getBackupEvent().emit("backup_delete", { backupUid: backup.backupUid, storage: "s3" });
        } catch (error) {
    ...
    copyBackupToS3 = async (metaBackup) => {
        try {
            ...
            if (res) {
                ...
                getBackupEvent().emit("backup_success", { ...metaBackup, storage: "s3" })
            } else {
                console.log("Error when sending file to wasabi");
                getBackupEvent().emit("backup_failed", { ...metaBackup, storage: "s3" })
            }
        } catch (err) {
    ...
    encryptedBackupToS3 = async (metaBackup) => {
        try {
            ...
            getBackupEvent().emit("backup_success", { ...metaBackup, storage: "s3", encrypted: true })
        } catch (error) {
            console.log("Error sending encrypted backup: ", error.message);
            getBackupEvent().emit("backup_failed", { ...metaBackup, storage: "s3", encrypted: true })
        }
    }
        ...
        if (archives.length > 0) {
            ...
            for (const archive of archives) { 
                try {
                    ...
                    getBackupEvent().emit("backup_delete", { backupUid: archive.backupUid, storagePath: archive.storagePath, storage: "s3" })
                } catch (err) {
