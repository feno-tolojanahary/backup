    ...
    removeS3Archive = async (nameId) => {
        try {
            ...
            if (backup.files?.length > 0) {
                removePromise = backup.files.map(file => this.deleteFile(file.storagePath))
            } 
    ...
    encryptedBackupToS3 = async (metaBackup) => {
        try {
            ...
            for (const { key, filePath } of metaFiles) {
                const res = await this.uploadFile({ filePath, key });
                if (!res) 
    ...
    removeS3Archives = async (archives) => {
        if (archives.length > 0) {
            ...
            for (const archive of archives) { 
                try {
                    const res = await this.deleteFile(archive.storagePath);
