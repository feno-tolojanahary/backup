...
const { getFormattedName, mergeByStoragePath } = require('../helper/utils');
...
const { resolveMongodbConf, searchConfig, resolveS3Config, resolveLocalConfig, resolveRemoteConfig, allSourceConfs, allDestinationConfs, getConfigurationsByTargetName } = require("../helper/mapConfig");
const LocalStorage = require("../storages/localStorage/localStorages");
const { testConf, downloadBackup, deleteBackup } = require("../storages/storageHelper");
...
exports.export = async (backupId, opts) => {
    try {
        ...
        if (backupInfo.encrypted) {
            ...
            exportPath = await oneDownloadFromAllStorage(backupInfo.storagePath, backupInfo.encrypted, dest);
            backupEvent.emit("download", backupInfo);
        }
