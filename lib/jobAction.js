...
async function execMongoJob(job) {
    try {
        ...
        for (const conf of activeConfigDests) {
            if (conf.type === "ssh") {
                const otherMeta = { storage: "remote", jobId: job.id, storageConf: confName };
                try {
                    ...
                    backupEvent.emit("backup_success", { ...backupInfo, ...otherMeta })
                } catch (error) {
                    backupEvent.emit("backup_failed", { ...dumpMetaData, ...otherMeta });
                }
            }
            if (conf.type === "s3") {
                const otherMeta = { storage: "s3", jobId: job.id, storageConf: confName };
                try {
                    ...
                    backup.emit("backup_success", { ...dumpMetaData, uploadedData, ...otherMeta })
                } catch (error) {
                    backup.emit("backup_failed", { ...dumpMetaData, ...otherMeta })
                }
            }
        }
        // Mark job as runned
