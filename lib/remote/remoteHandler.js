...
exports.removeArchive = async (name) => {
    const hostConf = getHostConfig();
    const remoteHost = new RemoteHost(hostConf);
    await remoteHost.connect();
    const resFind = await backupService.getAllBackups({ name })
    if (resFind.success && resFind.data.length > 0) {
        const backup = resFind.data[0];
        const res = await remoteHost.removeEntry(name, backup.isEncrypted);
        backupEvent.emit("backup_delete", { backupUid: backup.backupUid, storage: "remote" });

    } else {
        const isDir = !name.endsWith(".zip");
        const res = await remoteHost.removeEntry(name, isDir);
