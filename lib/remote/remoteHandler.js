...
const archiveFile = path.join(__dirname, "..", "data", "wasabi-remote.log");
...
exports.removeRemoteArchives = async (archives, hostName = null) => {
    ...
    for (const backupName of archives) {
        const res = await remoteHost.removeFile(backupName);
        if (res) {
            deletedBackupNames.push(res)
        }
    }
    if (deletedBackupNames.length > 0) {
        for (const backupName of deletedBackupNames) {
            dataArchiveLog += backupName + "\n";
        }
    }
    await fs.appendFile(archiveFile, dataArchiveLog);
}
...
exports.removeArchive = async (archiveName) => {
    ...
    const remoteHost = new RemoteHost(hostConf);
    await remoteHost.connect();
    const res = await remoteHost.removeFile(archiveName);
    await remoteHost.disconnect();
    return Boolean(res);
}
