...
class RemoteHandler {
    ...
    async uploadDir(dirPath, destFolder) {
        try {
            const destWorkingDir = await this.#openWorkingDirRemote(destFolder);
            if (!destWorkingDir) {
                return;
            }
            const dstPath = path.posix.join(remoteWorkingDir, path.basename(dirPath));
            await this.sftp.uploadDir(dirPath, destFolder);
            console.log("Uploading file to server with success")
            return {
                dstPath
            }
        } catch (error) {
    ...
    async removeFile(fileSubPath) {
        try {
            ...
            const res = await this.sftp.delete(filePath);
            return res;
        } catch (error) {
